...
{
    public class IpStatusDescriptionProvider
    {
        private readonly Dictionary<IPStatus, string> ipstatusDescriptions = new Dictionary<IPStatus, string>
        {
            ...
            { IPStatus.DestinationProhibited, "The ICMPv6 echo request failed because contact with the destination computer is administratively prohibited. This value applies only to IPv6." },
        ...
        public string GetDescription(IPStatus ipStatus)
        {
            if (ipstatusDescriptions.ContainsKey(ipStatus))
            {
                return ipstatusDescriptions[ipStatus];
            }
